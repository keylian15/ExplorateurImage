...
Toute écriture dans config.json remonte à MainWindow.save_workspaces()
via les signaux persist_requested (VMs) et history_changed (historique).
WorkspaceWidget ne détient jamais de copie de config ni n'écrit directement.
...
class WorkspaceWidget(QWidget):
    ...
    # Demande à MainWindow de sauvegarder — émis pour l'historique ET les VMs
    save_requested = pyqtSignal()
    # Demande à MainWindow de renommer l'onglet (lors d'une restauration historique)
    rename_requested = pyqtSignal(str, str)  # (ws_id, new_name)
    ...
    def __init__(
        ...
        # persist_requested des trois VMs -> _on_persist_requested
        self.gallery_vm.persist_requested.connect(self._on_persist_requested)
        self.detail_vm.persist_requested.connect(self._on_persist_requested)
        self.map_vm.persist_requested.connect(self._on_persist_requested)

        # ── Restauration du dossier ───────────────────────────────────────────
        if folder and os.path.exists(folder):
            self.gallery_vm.open_folder(folder)

    # ── Propriétés exposées à MainWindow ──────────────────────────────────────
    ...
    # ── Délégation de la persistance à MainWindow ─────────────────────────────

    def _request_save(self):
        """Point d'entrée unique : tout chemin qui veut écrire dans config passe ici."""
        self.save_requested.emit()

    def _on_persist_requested(self):
        """Relaye persist_requested des VMs vers MainWindow."""
        self._request_save()
    ...
    def on_folder_changed(self, old_folder: str, new_folder: str):
        if self._is_restoring_history:
            return

        self.history_tree.push(
            action_type=HistoryActionType.FOLDER_CHANGED,
            payload={"old_folder": old_folder, "new_folder": new_folder},
            active_view="gallery",
        )
        self._request_save()

        self.folder_changed.emit(self.ws_id, new_folder)
    ...
    def on_pin_changed(self, img_name: str, is_pinned: bool):
        ...
        self.history_tree.push(
            ...
            payload={"img_name": img_name, "is_pinned": is_pinned},
            active_view="gallery",
        )
        self._request_save()
    ...
    def on_history_back(self) -> None:
        current = self.history_tree.current
        if not current:
            return

        # Restaure l'état inverse du noeud courant
        self.restore_history_node(current)

        # Remonte dans l'arbre seulement si possible
        self.history_tree.back()
    ...
    def restore_history_node(self, node: HistoryNode) -> None:
        ...
        try:
            if node.action_type == HistoryActionType.SELECT:
                parent = node.parent
                if parent and parent.action_type == HistoryActionType.SELECT:
                    img_name = parent.payload.get("img_name")
                    if img_name:
                        self.gallery_vm.select_image(img_name)

            elif node.action_type == HistoryActionType.FOLDER_CHANGED:
                folder = node.payload.get("old_folder")
                if folder and os.path.exists(folder):
                    self.gallery_vm.open_folder(folder)
                    self.folder_changed.emit(self.ws_id, folder)

            elif node.action_type == HistoryActionType.RENAME_WORKSPACE:
                # Restaure l'ancien nom
                old_name = node.payload.get("old_name")
                print(old_name)
                if old_name:
                    self.ws_name = old_name
                    self._dock.setWindowTitle(f"Détails — {old_name}")
                    self.rename_requested.emit(self.ws_id, old_name)
